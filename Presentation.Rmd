...
base.data %>% 
  filter(year > 1945, sex == "MF", region_code == "Advanced Economies", country != "Luxembourg",
         country != "Turkey", country != "Portugal") %>% 
  mutate(color = case_when(country == "Germany" ~ "Germany",
                          ...
                          TRUE ~ NA_character_)) %>% 
  select(country, year, color, secad, ter) %>% 
  pivot_longer(names_to = "edulevel", values_to = "enrol", cols = 4:5) %>% 
  ggplot(aes(x = year, y = enrol)) +
  geom_line(alpha = 0.5, aes(group = country)) +
 # geom_smooth(aes(y = enrol, x = year)) +
  geom_line(data = . %>% filter(!is.na(color)),
            aes(x = year, y = enrol, group = country, color = color),
            size = 1.5) +
  facet_wrap(~edulevel)
